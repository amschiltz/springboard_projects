...
This project predicts telco customer churn using a reproducible, notebook-driven pipeline. The codebase is structured for clarity, interpretability, and academic rigor. **All major steps are in Jupyter notebooks, not scripts.**
...
```
- Data wrangling/feature engineering: `notebooks/01_*` → outputs to `data/processed/`
...
- Use `references/telco_column_role_manifest.csv` to define column roles/types (only use columns defined here for modeling)
- Save all plots to `reports/figures/` with descriptive, context-specific filenames
...
- EDA template: Target audit → Univariate → Bivariate → Multivariate (see `reports/eda_summary_template.md`)
...
- **Notebook-driven workflow:** All major steps are in Jupyter notebooks. Keep code modular and reproducible within notebooks.
...
- **Plotting:** Save all plots to `reports/figures/` with descriptive, context-specific filenames (not generic names).
...
For unclear or missing conventions, consult the above files or ask for clarification. If you discover new project-specific patterns, update this file with concise, actionable guidance.
